...
!(async() => {
    ...
    for(let i = 0; i < cookiesArr.length; i++){
        if(cookiesArr[i]){
            ...
            $.index = i + 1;
            message = "";
            console.log(`【账号】${$.index}.${$.UserName} 正在检测...`);
            await isLogin();
            await $.wait(1000);
        }
    }
        ...
        $.get(options, (err, resp, data) => {
            try{
                if(safeGet(data)){
                    ...
                    if(data.islogin === "1"){
                        console.log(`【账号】${$.index}.${$.UserName} ✅Cookie有效\n`)
                        // $.context += `【账号】${$.index}.${$.UserName} ✅Cookie有效\n`
                    } else if(data.islogin === "0"){
                        console.log(`【账号】${$.index}.${$.UserName} ❌CK已失效！！请在淘饭饭->我的->设置中,点3下版本重新提交\n`)
                        $.context += `【账号】${$.index}.${$.UserName} ❌CK已失效！！请在淘饭饭->我的->设置中,点3下版本重新提交\n`
                    } else {
                        console.log(`【账号】${$.index}.${$.UserName} ⚠️未知返回...\n`)
                        $.context += `【账号】${$.index}.${$.UserName} ⚠️未知返回...\n`
                    }
                }
            } catch(e){
                console.log(e);
